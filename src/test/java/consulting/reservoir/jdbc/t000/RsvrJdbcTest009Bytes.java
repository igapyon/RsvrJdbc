 ...
 * Float の確認。
 */
class RsvrJdbcTest009Bytes {
    private static final String CASEID = "test009";
    ...
    void test009() throws Exception {
        ...
        // 値その1 (insert)
        final byte[] value1 = "あいうえお".getBytes("Windows-31J");
        final Long recordId = insertInto(conn, value1);
        assertArrayEquals(value1, selectValue(conn, recordId));

        // 値その2 (update)
        final byte[] value2 = "かきくけこ".getBytes("Windows-31J");
        update(conn, recordId, value2);
        assertArrayEquals(value2, selectValue(conn, recordId));

        // 値その3 (null値)
        update(conn, recordId, null);
        assertArrayEquals(null, selectValue(conn, recordId));
    }
